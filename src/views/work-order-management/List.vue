    ...
    <div>
        ...
        >
            <template slot="btn-group">
                <el-button icon="el-icon-plus" @click.stop="handleVerifyState(1)">新建工单</el-button>
            </template>
                ...
                <el-table :data="tableData" @selection-change="handleSelectionChange" @row-dblclick="handleEdit">
                    ...
                    >
                        <template scope="scope">
                            {{scope.row.waybillNo ? scope.row.waybillNo :'-' }}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column
                            label="执行人">
                        <template scope="scope">
                            {{scope.row.senderName ? scope.row.senderName :'-' }}
                        </template>
                    </el-table-column>
                    ...
                    >
                        <template scope="scope">
                            {{scope.row.senderWorkNo ? scope.row.senderWorkNo :'-' }}
                        </template>
                    </el-table-column>
                    <el-table-column
                            label="问题描述"
                    ...
                    >
                        <template scope="scope">
                            {{scope.row.customerContact ? scope.row.customerContact :'-' }}
                        </template>
                    </el-table-column>
                    <el-table-column
                            label="联系人地址"
                    ...
                    >
                        <template scope="scope">
                            {{scope.row.customerAddress ? scope.row.customerAddress :'-' }}
                        </template>
                    </el-table-column>
                    <el-table-column
                            label="是否处理"
        ...
        <el-dialog
                ...
                width="40%">
            <el-form :inline="true" :model="solicitationForm" :rules="rules" ref="solicitationForm" label-width="100px">
                <el-row :gutter="20">
                    <el-col :span="24">
                        <el-form-item :label="'运单号'" prop="waybillNo">
                            <el-input v-model="solicitationForm.waybillNo">
                ...
                <el-row :gutter="20">
                    <el-col :span="24">
                        <el-form-item :label="'问题描述'" prop="remark">
                            <el-input type="textarea" v-model="solicitationForm.remark"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="执行人" prop="senderName">
                            <el-select @change="showSenderUser" v-model="solicitationForm.senderName"
                                       placeholder="请选择执行人">
                    ...
                    <el-col :span="12">
                        <el-form-item label="执行人电话" prop="senderWorkNo">
                            <el-input v-model="solicitationForm.senderWorkNo"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="联系人">
                            <el-input v-model="solicitationForm.customerName"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item label="联系人电话">
                            <el-input v-model="solicitationForm.customerContact"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="联系人地址">
                            <el-input v-model="solicitationForm.customerAddress"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item :label="'是否加急'">
                            <el-switch
                                    v-model="solicitationForm.isUrgent"
                                    active-value="1"
                                    inactive-value="0">
                            </el-switch>
                        </el-form-item>
            ...
            <el-button v-if="solicitationForm.status !='1'" type="primary" @click="updateDeliveryMethodTime('solicitationForm')">确 定</el-button>
          </span>
        </el-dialog>
    </div>
</template>
...
<script>
    export default {
        ...
        data() {
            return {
                api: {
                    list: this.$api.workOrder.list,
                    update: this.$api.workOrder.update,
                    tree: this.$api.building.tree,
                    matching: this.$api.workOrder.matching,
                ...
                rules: {
                    remark: [
                        {required: true, message: '请输入', trigger: 'remark'},
                    ],
                    senderName: [
        ...
        methods: {
            fetchFormat() {
                let that = this;
                that.$http.post(that.api.matching, {waybillNo: that.solicitationForm.waybillNo}).then((resp) => {
                    if (resp.data.status === 200) {
                        let data = resp.data.data.workOrder;
                        that.solicitationForm.senderName = data.senderWorkNo
                        that.solicitationForm.senderWorkNo = data.senderWorkNo
                        that.solicitationForm.customerContact = data.customerContact
                        that.solicitationForm.customerName = data.customerName
                        that.solicitationForm.customerAddress = data.customerAddress
                        that.$message({
                            type: 'success',
            ...
            updateDeliveryMethodTime(form) {
                let vm = this;
                this.$refs[form].validate((valid) => {
                    // 判断必填项
                    if (valid) {
                        vm.showSender = true;
                        this.$http.post(this.api.update, vm.solicitationForm).then((resp) => {
                            if (resp.data.status === 200) {
                                vm.showSender = false;
            ...
            handleSearch(index) {
                ...
                this.$http.post(this.api.list, {}).then((resp) => {
                    vm.tableData = resp.data.data.workOrder;
                    //处理分页
